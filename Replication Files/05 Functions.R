# Mediation function (Imai et al, 2010) for panel data analysis
# Required dependencies: sandwich, pcse
suppressMessages(library(pcse))

panel_mediate_pcse=function (model.m, model.y, groupN, groupT, sims = 1000, boot = FALSE, boot.ci.type = "perc", 
          ...
          cluster = NULL, group.out = NULL, use_speed = FALSE, ...) 
{
  ...
  if (isMer.m && class(model.m)[[1]] == "glmerMod") {
    ...
    else if (m.family[1] == "poisson" && m.family[2] == 
             "identity") {
  ...
  if (isMer.y && class(model.y)[[1]] == "glmerMod") {
    ...
    else if (y.family[1] == "poisson" && y.family[2] == 
             "identity") {
  ...
  if (!isOrdered.y) {
    if (!boot) {
      ...
      if (isOrdered.m) {
        ...
        if (robustSE & !isMer.m) {
          ...
          MModel.var.cov <- sandwich::vcovCL(fm, dta[, 
                                                     ncol(dta)])
        }
        else {
          MModel.var.cov <- vcov(model.m)
      ...
      if (isRq.y) {
        ...
        if (robustSE & !isMer.y) {
          ...
          YModel.var.cov <- sandwich::vcovCL(fm, dta[, 
                                                     ncol(dta)])
        }
        else {
          YModel.var.cov <- vcov(model.y)
      ...
      if (isGlm.m) {
        ...
        m.cat <- sort(unique(model.frame(model.m)[, 
                                                  1]))
        lambda <- model.m$zeta
        mmat.t <- mmat.t[, -1]
        mmat.c <- mmat.c[, -1]
      ...
      D <- boot::boot(data = y.data, statistic = med.fun, 
                      R = sims, sim = "ordinary", m.data = m.data, 
                      ...)
      ...
      z1 <- D["z1"]
      z0 <- D["z0"]
      list2env(lapply(list(d1 = d1, d0 = d0, z1 = z1, 
                           z0 = z0, delta.1 = delta.1, delta.0 = delta.0, 
                           zeta.1 = zeta.1, zeta.0 = zeta.0), unname), 
               envir = environment())
    ...
    if (isMer.y | isMer.m) {
      ...
      for (g in 1:G) {
        ...
        n.avg.p.group[g] <- pval(nu.avg.group[g, ], 
                                 n.avg.group[g])
    ...
    if (!long && (isMer.y || isMer.m)) {
      out <- list(d0 = d0, d1 = d1, d0.ci = d0.ci, d1.ci = d1.ci, 
                  ...
                  d.avg.p = d.avg.p, d.avg.ci = d.avg.ci, z.avg = z.avg, 
                  z.avg.p = z.avg.p, z.avg.ci = z.avg.ci, n.avg = n.avg, 
                  n.avg.p = n.avg.p, n.avg.ci = n.avg.ci, d0.group = d0.group, 
                  d1.group = d1.group, d0.ci.group = d0.ci.group, 
                  d1.ci.group = d1.ci.group, d0.p.group = d0.p.group, 
                  d1.p.group = d1.p.group, z0.group = z0.group, 
                  z1.group = z1.group, z0.ci.group = z0.ci.group, 
                  z1.ci.group = z1.ci.group, z0.p.group = z0.p.group, 
    ...
    D <- boot::boot(data = y.data, statistic = med.fun.ordered, 
                    R = sims, sim = "ordinary", m.data = m.data, ...)
    ...
    D <- med.fun.ordered(y.data = y.data, m.data = m.data, 
                         index = 1:n)
    d1 <- D[col_index[[1]]]
    d0 <- D[col_index[[2]]]
    z1 <- D[col_index[[3]]]
...
# pval dependency for Imai et al's (2010) algorithm
...
## code by stackoverflow "Vincent
# URL: https://stackoverflow.com/questions/5541010/xtpcse-from-stata-how-to-rewrite-in-r
...
#########################################################
######## FIXED-EFFECTS PRAIS-WINSTEN REGRESSION #########
#########################################################

# Calculating rho as in Cochrane and Orcutt Regression
rho.Y_1.get = function(object, groupN){
  if(!"lm" %in% class(object)){stop("Formula object must be of class 'lm'.")}
  groupN = tail(as.character((match.call()$groupN)), 1)
  #Obtain Residual data
  resid = data.frame(object$residuals)
  colnames(resid)[1] = "residuals"
  resid$unit = object$model[,groupN]
  #Create lagged residuals
  resid = resid %>%
    dplyr::group_by(unit) %>%
    dplyr::mutate(
      lagged = dplyr::lag(residuals, n = 1)
    ) %>%
    ungroup()
  #Create lm for residuals
  resid_lm = lm(residuals ~ lagged,
                data = resid)
  
  # Create Cochrane-Orcutt's rho
  CO_rho=(resid_lm$coefficients[[2]])^2
  
  #Create Prais-Winsten's Y_1 weight
  PW_rho = sqrt(1 - (resid_lm$coefficients[[2]])^2)
  
  # Calculation of rhos
  rho.Y_1=list(CO_rho,PW_rho)
  return(rho.Y_1)
  
}

# for mediate function with Newey-West HAC
panel_mediate_nwHAC=function (model.m, model.y, sims = 1000, boot = FALSE, boot.ci.type = "perc", 
                             treat = "treat.name", mediator = "med.name", covariates = NULL, 
                             outcome = NULL, control = NULL, conf.level = 0.95, control.value = 0, 
                             treat.value = 1, long = TRUE, dropobs = FALSE, robustSE = FALSE, 
                             cluster = NULL, group.out = NULL, use_speed = FALSE, ...) 
  ...
  if (!isOrdered.y) {
    if (!boot) {
      ...
      if (isOrdered.m) {
        ...
        if (robustSE & !isMer.m) {
          MModel.var.cov <- sandwich::vcovHAC(model.m)
        }
        else if (robustSE & isMer.m) {
          MModel.var.cov <- vcovHC(model.m, ...)
          warning("robustSE does not support mer class: non-robust SEs are computed for model.m")
      ...
      if (isRq.y) {
        ...
        if (robustSE & !isMer.y) {
          YModel.var.cov <- sandwich::vcovHAC(model.y)
        }
        else if (robustSE & isMer.y) {
          YModel.var.cov <- vcov(model.y)
          warning("robustSE does not support mer class: non-robust SEs are computed for model.y")
...
panel_mediate_vcovPL=function (model.m, model.y, groups, aggregate_PL, sims = 1000, boot = FALSE, boot.ci.type = "perc", 
                               treat = "treat.name", mediator = "med.name", covariates = NULL, 
                               outcome = NULL, control = NULL, conf.level = 0.95, control.value = 0, 
                               treat.value = 1, long = TRUE, dropobs = FALSE, robustSE = FALSE, 
                               cluster = NULL, group.out = NULL, use_speed = FALSE, ...) 
{
  groups <- tail(as.character((match.call()$groups)), 1)
  aggregate_PL <- tail(as.character((match.call()$aggregate_PL)), 1)
  cl <- match.call()
  if (match("INT", names(cl), 0L)) {
    warning("'INT' is deprecated - existence of interaction terms is now automatically detected from model formulas")
  }
  ...
  if (!isOrdered.y) {
    if (!boot) {
      ...
      if (isOrdered.m) {
        ...
        if (robustSE & !isMer.m) {
          MModel.var.cov <- sandwich::vcovPL(model.m,
                                             cluster=groups,
                                             aggregate=aggregate_PL)
        }
        else if (robustSE & isMer.m) {
          MModel.var.cov <- vcovHC(model.m, ...)
          warning("robustSE does not support mer class: non-robust SEs are computed for model.m")
      ...
      if (isRq.y) {
        ...
        if (robustSE & !isMer.y) {
          YModel.var.cov <- sandwich::vcovPL(model.y,
                                             cluster=groups,
                                             aggregate=aggregate_PL)
        }
        else if (robustSE & isMer.y) {
...
panel_mediate_vcovCL=function (model.m, model.y, group.cluster, type_HC, sims = 1000, boot = FALSE, boot.ci.type = "perc", 
                               treat = "treat.name", mediator = "med.name", covariates = NULL, 
                               outcome = NULL, control = NULL, conf.level = 0.95, control.value = 0, 
                               treat.value = 1, long = TRUE, dropobs = FALSE, robustSE = FALSE, 
                               cluster = NULL, group.out = NULL, use_speed = FALSE, ...) 
{
  group.cluster <- tail(as.character((match.call()$group.cluster)), 1)
  type_HC <- tail(as.character((match.call()$type_HC)), 1)
  cl <- match.call()
  if (match("INT", names(cl), 0L)) {
    warning("'INT' is deprecated - existence of interaction terms is now automatically detected from model formulas")
  }
  ...
  if (!isOrdered.y) {
    if (!boot) {
      ...
      if (isOrdered.m) {
        ...
        if (robustSE & !isMer.m) {
          MModel.var.cov <- sandwich::vcovCL(model.m,
      ...
      if (isRq.y) {
        ...
        if (robustSE & !isMer.y) {
          YModel.var.cov <- sandwich::vcovCL(model.y,
                                             cluster=group.cluster,
                                             type=type_HC)
        }
        else if (robustSE & isMer.y) {
...
panel_mediate_vcovPL_fixed=function (model.m, model.y, sims = 1000, boot = FALSE, boot.ci.type = "perc", 
                               treat = "treat.name", mediator = "med.name", covariates = NULL, 
                               outcome = NULL, control = NULL, conf.level = 0.95, control.value = 0, 
                               treat.value = 1, long = TRUE, dropobs = FALSE, robustSE = FALSE, 
                               cluster = NULL, group.out = NULL, use_speed = FALSE, ...) 
...
panel_mediate_vcovCL_fixed=function (model.m, model.y, sims = 1000, boot = FALSE, boot.ci.type = "perc", 
                               treat = "treat.name", mediator = "med.name", covariates = NULL, 
                               outcome = NULL, control = NULL, conf.level = 0.95, control.value = 0, 
                               treat.value = 1, long = TRUE, dropobs = FALSE, robustSE = FALSE, 
                               cluster = NULL, group.out = NULL, use_speed = FALSE, ...) 
    ...
    if (!boot) {
      ...
      if (isOrdered.m) {
        ...
        if (robustSE & !isMer.m) {
          MModel.var.cov <- sandwich::vcovCL(model.m,
                                             cluster=~country,
                                             type="HC0") #Set to White's default
        }
        else if (robustSE & isMer.m) {
          MModel.var.cov <- vcovHC(model.m, ...)
          warning("robustSE does not support mer class: non-robust SEs are computed for model.m")
...
tsls_mediate_vcovPL_fixed = function (model.m, model.y, treat = "treat.name", conf.level = 0.95, 
  ...
  if (!est_se) {
    ...
    if (!boot) {
      ...
      if (!is.null(cluster)) {
        ...
        vcv_y <- sandwich::vcovPL(mod.y, cluster=~country+year, aggregate=FALSE, ...)
        vcv_m <- sandwich::vcovHC(model.m, cluster=~country+year, aggregate=FALSE, ...)
      }
      else {
        vcv_y <- vcov(mod.y)
        vcv_m <- vcov(model.m)
